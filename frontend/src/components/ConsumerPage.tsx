...
function ConsumerPage() {
  ...
  const [decryptedMedicalRecords, setDecryptedMedicalRecords] = useState<
    string | undefined
  >("");
  const [decryptedFinancialRecords, setDecryptedFinancialRecords] = useState<
    string | undefined
  >("");
  const [decryptedDnrRecords, setDecryptedDnrRecords] = useState<
    string | undefined
  >("");
  const [decryptedGenomicRecords, setDecryptedGenomicRecords] = useState<
    string | undefined
  >("");
  ...
  const handleIpfsUrlChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    >
      ...
      <Stack spacing={4}>
        ...
        <Input
          ...
          value={ipfsUrl}
          onChange={handleIpfsUrlChange}
        />
        <Button
          w={300}
          colorScheme="purple"
          onClick={parseMessage}
          isDisabled={ipfsUrl?.length <= 0}
        >
          Process data
        </Button>

        <Text fontWeight={600}>Encrypted Data:</Text>
        <Textarea placeholder="Encrypted Data" value={encryptedData} readOnly />
      </Stack>

      <Stack>
        <Text fontWeight={600}>Encrypted Genomic Records: </Text>
        <Input placeholder="Medical Records" value={eMedicalRecords} readOnly />
        <Button w={300} colorScheme="purple" onClick={decryptMedicalRecords}>
          Decrypt Genomic Records
        </Button>
        <Text fontWeight={600}>Decrypted Genomic Records: </Text>
        {!decryptedMedicalRecords || decryptedMedicalRecords === "" ? (
          ""
        ) : (
          <JsonView src={JSON.parse(decryptedMedicalRecords)} collapsed />
        )}
      </Stack>
      ...
      <Stack>
        ...
        <Input placeholder="Medical Records" value={eMedicalRecords} readOnly />
        <Button w={300} colorScheme="purple" onClick={decryptFinancialRecords}>
          Decrypt Financial Records
        </Button>
        <Text fontWeight={600}>Decrypted Financial Records: </Text>
        {!decryptedFinancialRecords || decryptedFinancialRecords === "" ? (
          ""
        ) : (
          <JsonView src={JSON.parse(decryptedFinancialRecords)} collapsed />
        )}
      </Stack>

      <Stack>
        <Text fontWeight={600}>Encrypted DNR Records: </Text>
        <Input placeholder="DNR Records" value={eDnrRecords} readOnly />
        <Button w={300} colorScheme="purple" onClick={decryptDnrRecords}>
          Decrypt DNR Records
        </Button>
        <Text fontWeight={600}>Decrypted DNR Records: </Text>
        {!decryptedDnrRecords || decryptedDnrRecords === "" ? (
          ""
        ) : (
          <JsonView src={JSON.parse(decryptedDnrRecords)} collapsed />
        )}
      </Stack>

      <Stack>
        <Text fontWeight={600}>Encrypted Genomic Records: </Text>
        <Input placeholder="Genomic Records" value={eGenomicRecords} readOnly />
        <Button w={300} colorScheme="purple" onClick={decryptGenomicRecords}>
          Decrypt Genomic Records
        </Button>
        <Text fontWeight={600}>Decrypted Genomic Records: </Text>
        {!decryptedGenomicRecords || decryptedGenomicRecords === "" ? (
          ""
        ) : (
          <JsonView src={JSON.parse(decryptedGenomicRecords)} collapsed />
        )}
      </Stack>

      <Stack>
        <Text fontWeight={600}>Encrypted De-Identified Records: </Text>
        <Input placeholder="MyDID" value={eMyDID} readOnly />
        <Button w={300} colorScheme="purple" onClick={decryptMyDID}>
          Decrypt De-Identified Records
        </Button>
        <Text fontWeight={600}>Decrypted De-Identified Records: </Text>
        {!decryptedMyDID || decryptedMyDID === "" ? (
          ""
        ) : (
          <JsonView src={JSON.parse(decryptedMyDID)} collapsed />
        )}
      </Stack>
    </Stack>
  );
